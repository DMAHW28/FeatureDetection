...
using namespace std;
namespace desc{
    tuple<cv::Mat, vector<cv::Point>> extract_descriptors(const cv::Mat& image, const vector<cv::Point>& points, int window_size = 3);
    ...
    tuple<vector<cv::Point>, vector<cv::Point>, vector<cv::Point>> compute_matching_points(const string& img1_file, const string& img2_file, const vector<cv::Point>& pts1, const vector<cv::Point>& pts2, int windows_size = 3, const string& metric = "euclidean", const string& match = "ratio", float ratio_threshold=0.8);
